...
I first filtered out all the species with less than 3 individuals for which measurements were taken for each morphometric trait. I hypothesised that a smaller sample size than 3 is not enough to ensure the species means are reflective of the entire population, especially if among those 3 individuals only 1 or 2 measurements were taken for my studied traits. I also excluded species with missing centroid values. Finally, I excluded species for which the Wing Length and Secondary Length were inferred from adjacent species, as this would make detecting the presence of a gradient for those traits across a family of species more difficult and could skew results. Finally, I removed duplicates in the excluded data.

Some sanity checks were then conducted, ensuring that column values were as expected. Based on the summary, I investigated some of the outliers in the data, such as the species with the highest mass, 
...
to_exclude = avonet %>%
  filter(str_detect(Traits.inferred, "Wing Length") | str_detect(Traits.inferred, "Secondary1")) %>%
  mutate(
    date="19/10/23",
...
#   MEANS AND MEDIANS OF EVERYTHING
#   rowwise() %>%
