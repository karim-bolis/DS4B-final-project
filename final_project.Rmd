...
Assessing the families with the most correlation (by correlation coefficient) shows the  Remizidae family has the largest negative trend for Wing Length and Kipps' Distance, explaining 61-62% of the trait's variation through the mean annual temperature (r=-0.78^2 --> R=0.61). Predictably, it is one of the highest negatively correlated families for Hand-Wing Index as well. Musophagidae also occurs twice for showing the largest positive trend for the Hand-Wing Index and Kipp's Distance.

Frohlich et al. found that various configurations of body size and appendage size allow organisms to adapt to climates. (Frohlich et al. 2023) In their findings, tarsus size actually decreased with temperature in smaller birds, producing the opposite effect to Allen's rule, while larger birds were more likely to obey the rule. (Frohlich et al. 2023) Indeed, Allen himself hypothesised that larger birds would be more likely to follow his rule. (Frohlich et al. 2023) The vastly different masses of Remizidae and Musophagidae birds support this. Additionally, Xu et al. found that passerines (Remizidae) exhibit a negative trend with annual mean temperature, while non-passerines (Musophagidae) exhibit a positive trend for bill and tarsus length. (Xu 2023) It is possible, then, that wing length is under a similar selection pressure to that producing these trends in tarsus length.
...
print(arrange(filter(correlations_p_r_values,Trait=="Hand_Wing_Index"),r_value))

mean(filter(avonet,Family3=="Remizidae")$Mass)
mean(filter(avonet,Family3=="Musophagidae")$Mass)
