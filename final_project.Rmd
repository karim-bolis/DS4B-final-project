...
avonet_birdtree = avonet_birdtree %>%
  mutate(row_id = row_number()) %>%
  select(row_id, everything())

write.csv(avonet_birdtree, file = "processed_data/tidied_data.csv", row.names = FALSE)

excluded_data <- read.csv("processed_data/excluded_data.csv")
tidied_data <- read.csv("processed_data/tidied_data.csv")
cleaned_data <- tidied_data %>%
  anti_join(excluded_data, by = "row_id")

write.csv(cleaned_data, file = "processed_data/cleaned_data.csv", row.names = FALSE)
...
r <- getData("worldclim",var="bio",res=2.5, path="raw_data")
r <- r[[1]]
# names(r) <- "Temp"
# plot(r)
